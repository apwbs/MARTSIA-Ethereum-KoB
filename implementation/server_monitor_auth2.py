...
def send_ipfs_link(reader_address, process_instance_id, hash_file):
    nonce = web3.eth.getTransactionCount(authority2_address)
    ...
    signed_tx = web3.eth.account.sign_transaction(tx, authority2_private_key)
...
def transactions_monitoring():
    ...
    while True:
        ...
        for transaction in block.transactions:
            if transaction['to'] == authority2_address and transaction['hash'] not in transactions \
